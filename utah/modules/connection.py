...
from utah import dispatcher, LOGGER, SUDO_USERS, spamcheck
from utah.modules.helper_funcs.chat_status import bot_admin, user_admin, is_user_admin, can_restrict
from utah.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from utah.modules.helper_funcs.string_handling import extract_time

from utah.modules import languages
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                ...
                try:
                    ...
                    context.bot.send_message(update.effective_message.from_user.id, languages.tl(update.effective_message, "Anda telah terhubung dengan *{}*. Gunakan /connection untuk informasi perintah apa saja yang tersedia.").format(chat_name), parse_mode="markdown")
                except BadRequest:
                    pass
