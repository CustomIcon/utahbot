...
from utah import dispatcher, spamcheck, LOGGER
from utah.modules.disable import DisableAbleCommandHandler
from utah.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from utah.modules.sql import languages_sql as sql
...
for x in os.listdir('utah/modules/langs'):
	if os.path.isdir('utah/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("utah.modules.langs." + x)
