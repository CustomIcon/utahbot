...
import utah.modules.sql.locks_sql as sql
from utah import dispatcher, spamcheck, SUDO_USERS, LOGGER
...
def locktypes(update, context):
    ...
    send_message(
        ...
        + "\n\n"
        + "\n - ".join(
            [tl(update.effective_message, "*Jenis izin kunci yang tersedia adalah:* ")]
            + perm
        ),
        parse_mode="markdown",
    )
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    text = tl(
                        update.effective_message,
                        "Terkunci pesan *{}* untuk semua non-admin!",
                    ).format(ltype)
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace(
            tl(update.effective_message, "obrolan ini"), "*{}*".format(chat_name)
        )
